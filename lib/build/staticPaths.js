...
import { isRuntimeIsrEnabled } from '@/lib/build/isrPolicy'
...
export async function getStaticPathsBase({
  ...
  // Hobby-safe default: build every known page and reject unknown paths at
  // the CDN. Runtime fallback can be re-enabled explicitly for sites that
  // accept the additional Function and ISR usage.
  if (!isRuntimeIsrEnabled()) {
    return {
      paths: allPages.filter(filterFn).map(mapPageToParams),
      fallback: false
    }
